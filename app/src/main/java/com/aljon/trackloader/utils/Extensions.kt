...
/**
 * Navigate with checker if navDirection can be perform by the current fragment/destination
 *
 * @param direction - fragment navigation directions
 * @param extras - navigator extras that contains shared elements for transition
 */
...
/**
 * Delays the enter transition of the fragment until the target view is about to be drawn
 *
 * @param targetView - view to be drawn
 */
...
fun String.toTimeElapsed(): String {
    val then = LocalDateTime.parse(this, DateTimeFormatter.ofPattern(Constants.PREF_DATE_TIME_PATTERN))
